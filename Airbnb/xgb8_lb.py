...
df_all.to_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/df_all_4.csv',index=False)
del df_all
...
y_pred_test2 = np.zeros((62096,12))
y_pred_test_groups1 = np.zeros((62096,12))  
y_pred_test_groups2 = np.zeros((62096,12))  
y_pred_test_groups3 = np.zeros((62096,12))  
err_list = []

df_all_new = pd.read_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/df_all_4.csv')

idx_age = []
cts_age = []
for age_gender_el in df_all_new['age_group'].unique():
    print age_gender_el
    ...
    df_train = df_train[df_train['age_group']==int(age_gender_el)]
    df_test = df_test[df_test['age_group']==int(age_gender_el)]
    print df_test
    print df_test.shape
    y_pred_test = np.zeros((df_test.shape[0],12))
        
    id_test_age = df_test['id']
    print id_test_age
    print df_test['sum_zeros'].sum()
    ...
    ohe_feats = ['gender', 'signup_method', 'language', 'affiliate_channel', 'affiliate_provider',
                                     'first_affiliate_tracked', 'signup_app', 'first_device_type', 'first_browser', 'age_gender', 'age_group', 'dac_day', 'dac_month', 'dac_year']
    ...
    print id_test_age
    #Taking the 12 classes with highest probabilities
    ids = []  #list of ids
    ...
    print id_test_age.shape
    print y_pred_test.shape
    for j in id_test_age:
        #idx = id_test_age[j]
        idx = j
        ids += [idx] * 1
        cts1 += y_pred_test[i].tolist()[0]
        cts2 += y_pred_test[i].tolist()[1]
        cts3 += y_pred_test[i].tolist()[2]
        cts4 += y_pred_test[i].tolist()[3]
        cts5 += y_pred_test[i].tolist()[4]
        cts6 += y_pred_test[i].tolist()[5]
        cts7 += y_pred_test[i].tolist()[6]
        cts8 += y_pred_test[i].tolist()[7]
        cts9 += y_pred_test[i].tolist()[8]
        cts10 += y_pred_test[i].tolist()[9]
        cts11 += y_pred_test[i].tolist()[10]
        cts12 += y_pred_test[i].tolist()[11]
        i = i + 1
    idx_age.extend(ids)
    cts_age.extend(cts)
    del df_train
...
sub_age = pd.DataFrame(np.column_stack((idx_age, cts1, cts2, cts3, cts4, cts5, cts6, cts7, cts8, cts9, cts10, cts11, cts12)), columns=['id', 'pred1', 'pred2', 'pred3', 'pred4', 'pred5', 'pred6',  'pred7', 'pred8', 'pred8', 'pred10', 'pred11', 'pred12']]])
print ("sub_age created...")
print sub_age.shape
df_test = pd.read_csv('C:/Users/1/Desktop/kaggle/airbnb/data/test_users.csv')
sub_age.reindex(df_test.index)
del df_test
y_pred_test_groups1 = np.asarray(cts)
print y_pred_test_groups1.shape
...
for i in range(num_cv):
    print i
    df_all_new = pd.read_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/df_all_4.csv')
    df_all_new = df_all_new.drop(['age_group'], axis=1)
    ...
    #Label-encoding features
    ohe_feats = ['gender', 'signup_method', 'language', 'affiliate_channel', 'affiliate_provider',
                                     'first_affiliate_tracked', 'signup_app', 'first_device_type', 'first_browser', 'age_gender', 'dac_day', 'dac_month', 'dac_year']
...
sub.to_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/5_90_10_lb.csv',index=False)
...
#Generate submission
sub = pd.DataFrame(np.column_stack((ids, cts)), columns=['id', 'country'])
sub.to_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/5_80_20_lb.csv',index=False)
...
#Generate submission
sub = pd.DataFrame(np.column_stack((ids, cts)), columns=['id', 'country'])
sub.to_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/5_70_30_lb.csv',index=False)
