...
df_train = pd.read_csv('C:/Users/1/Desktop/kaggle/airbnb/data/train_users_sessions4.csv')
df_test = pd.read_csv('C:/Users/1/Desktop/kaggle/airbnb/data/test_users_sessions4.csv')
...
df_all['dac_year']=df_all['dac_year'].apply(int)
df_all['dac_month']=df_all['dac_month'].apply(int)
df_all['dac_day']=df_all['dac_day'].apply(int)
...
"""
df_all['tfa_year'].fillna(df_all['tfa_year'].median(),inplace = True);
df_all['tfa_month'].fillna(df_all['tfa_month'].median(),inplace = True);
df_all['tfa_day'].fillna(df_all['tfa_day'].median(),inplace = True);
...
df_all['tfa_temp'] = df_all['tfa_year'] + '-' + df_all['tfa_month'] + '-' + df_all['tfa_day']

df_all['tfa_weekday'] = pd.DatetimeIndex(df_all['tfa_temp']).weekday

av = df_all.tfa_weekday.values
df_all['tfa_weekday']  = np.where(np.logical_or(av == 5, av == 6), 1, 0)

df_all = df_all.drop(['tfa_temp'], axis=1)

df_all['tfa_year']=df_all['tfa_year'].apply(int)
df_all['tfa_month']=df_all['tfa_month'].apply(int)
df_all['tfa_day']=df_all['tfa_day'].apply(int)
df_all['tfa_hour']=df_all['tfa_hour'].apply(int)
"""
#Create summer/winter feature
av = df_all.dac_month.values
df_all['dac_summer_winter']  = np.where(np.logical_and(av >= 5, av <= 9), 1, 0)
...
rand_airbnb = np.random.randint(average_age - std_age,average_age + std_age,size = count_nan_age);

df_all['age'][np.isnan(df_all['age'])] = rand_airbnb;
...
del df_train
del df_test
df_all.to_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/df_all.csv',index=False)
del df_all

for i in range(num_cv):
    print i
    df_all_new = pd.read_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/df_all.csv')
...
for j in range(len(id_test)):
    idx = id_test[j]
    ids += [idx] * 12
    cts += le.inverse_transform(np.argsort(y_pred_test[j])[::-1])[:12].tolist()

#Generate submission
sub = pd.DataFrame(np.column_stack((ids, cts)), columns=['id', 'country'])
sub.to_csv('C:/Users/1/Desktop/kaggle/airbnb/temp/1_6.csv',index=False)
