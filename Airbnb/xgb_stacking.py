...
clf7=KNeighborsClassifier(n_neighbors=320, weights='uniform', algorithm='auto', leaf_size=30,
                              p=2, metric='minkowski', metric_params=None)                    
"""
#clf1=RandomForestClassifier(n_estimators=25, criterion='gini') 
                            
#clf2=ExtraTreesClassifier(n_estimators=25, criterion='gini')

#clf3=GradientBoostingClassifier(n_estimators=45, loss='deviance')
clf1=KNeighborsClassifier(n_neighbors=5)
...
for i in range(num_cv):
    ...
    #Stacking
    train_blend, test_blend, train_probs, test_probs = StackModels(df_train, df_test, y, clfs, 4)     
    ...
    X_train, X_test, y_train, y_test = cross_validation.train_test_split(train_blend, y, test_size=0.1, random_state=np.random.randint(10,1000))
    dtrain = xgb.DMatrix(X_train, y_train)
    dvalid = xgb.DMatrix(X_test, y_test)
    dtest = xgb.DMatrix(test_blend)
...
sub.to_csv('xgb_stacking_2.csv',index=False)
print err_list
